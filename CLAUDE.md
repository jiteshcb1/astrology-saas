...
- **SP-2 NEXT:** 2.3 branding (`org_branding` + catalog-constrained theme/font/locale + live preview) and 2.4 payment setup (`payment_methods` + envelope-encrypted BYO keys + test-connection). Their checklist/settings items currently show "Soon". Consultant nav items Packages/Bookings/Availability/Payments/Team/Seekers are disabled "Soon" until SP-3+.

### Tenant-scoped models registered (in `config/tenant-models.json`): `orgMember`, `receipt`, `consultantProfile`. Platform-level (NOT registered, accessed by super-admin/platform): `organization` (tenant root), `subscription`, `subscriptionPlan`, `feature_flags`, `billing_events`, `catalog_items`, `audit_logs`.
