...
- **SP-2.4 payment setup** (done, green): `app/dashboard/settings/payments` (`components/dashboard/PaymentForm.tsx`) — **the platform never touches funds**. Two modes: **UPI QR + manual proof** (VPA + QR image upload, R2 object key, stub-safe) and **BYO Razorpay (manual keys)** (paste `key_id`/`key_secret`). Logic in **`lib/payments.ts`** (cores `saveUpiCore`/`saveGatewayCore`/`testConnectionCore` + `toSafeView` + `isValidVpa` + `PaymentConnection` interface for future OAuth) and **`lib/crypto.ts`** (envelope encryption: per-row AES-256-GCM DEK wrapped by a KEK derived from `ENCRYPTION_MASTER_KEY`; `encryptSecret`/`decryptSecret`/`isEncryptionConfigured`; master key read from `process.env` at call time). **Secret handling (enforced):** key_id+key_secret stored `_enc` (never plaintext); `toSafeView` strips all `*_enc` and exposes only a masked `keyIdLast4` (write-only — secret never returned/prefilled, "Replace keys" to re-enter); audit `payment.update` metadata records only `{mode, provider}` (never key material); **`decryptSecret` is called ONLY in `testConnectionCore`** (and reserved for the SP-4.3 charge flow) — never at render. Test-connection = `lib/razorpay.ts` `testConnection` (harmless server-side REST read via `fetch`, no SDK; returns only `{ok,message}`). New table **`payment_methods`** (1:1 org, tenant-scoped); `connectionType`/`oauthTokenEnc` reserved for Phase-2 OAuth. Settings hub Payments card + dashboard checklist "Set up payments" now live (done when a `payment_method` exists).
- **SP-2 COMPLETE.** (Deferred: OAuth "Connect" for gateways; real charge/order+webhook flow is SP-4.3; managed KMS later.)

### In progress — Phase 1, SP-3 (Scheduling Engine)
- **SP-3 Pass 1** (done, green): the **no-double-book engine** + consultant availability + packages + slot computation. **6 new tenant tables** (`availability_schedules`/`availability_rules`/`availability_overrides`/`packages`/`bookings`/`booking_slots`), all carrying `organizationId` and registered. **The hard invariant lives in the DB:** migration `booking_engine` adds `CREATE EXTENSION btree_gist` + `EXCLUDE USING gist ("hostMemberId" WITH =, tstzrange("startsAt","endsAt",'[)') WITH &&) WHERE (active)` on `booking_slots` (constraint `booking_slots_no_overlap`). Range columns are Prisma-managed (no `Unsupported`); the constraint operates on the range expression. A conflicting insert raises SQLSTATE **23P01**.
  - **`lib/scheduling.ts`**: `getAvailableSlots()` (free intervals − active slots ± buffers − below-min-notice − stepped by interval − frequency caps; variable durations) and **`reserveSlot()`** — `tenantTransaction` creating `Booking`+`BookingSlot`, catching `isExclusionViolation` (23P01) → `{ok:false, reason:"slot_taken"}`. The DB is the arbiter; no app check-then-insert.
  - **`lib/availability.ts`**: `computeFreeIntervals()` (pure) + `getDefaultSchedule`/`saveAvailabilityCore` (audit `availability.update`). **`lib/timezone.ts`**: Intl-based `zonedClockToUtc`/`utcToZonedParts`/`weekdayOf`/`eachDateISO` (no dep; DST-correct). **`lib/packages.ts`**: `savePackageCore`/`setPackageActiveCore`/`deletePackageCore` (delete guarded when bookings exist) + `slugify` + `parseDurations` + `validatePackageInput`; price in paise; limits (buffers/notice/interval/freq) on the package.
  - UI: `/dashboard/availability` (`AvailabilityEditor` — weekday ranges, copy-to-all, overrides, tz) + `/dashboard/packages` (list + `new`/`[id]` editor, `PackageForm`). Packages/Availability nav items live; checklist "Create your first package" actionable.
  - Tests: `timezone`, `availability` (pure) + DB-gated `scheduling` (**concurrency: two overlapping `reserveSlot` → exactly one wins**; variable 60-vs-15 overlap rejected; slot computation honors buffers/notice/freq) + `packages`. NOTE: availability **limits live on the package** (per docs/4), not the schedule (the mockup groups them differently).
- **SP-3 NEXT / deferred:** `package_questions` (3.4 — SP-4 form); round-robin (SP-5 — single-host now, `assignmentMode` reserved); Google Calendar free/busy subtraction (Track I — `getAvailableSlots` hook); public booking page + payment + slot-hold expiry (SP-4 calls `reserveSlot`). `bookings.seekerUserId` is nullable until SP-4.

### Tenant-scoped models registered (in `config/tenant-models.json`): `orgMember`, `receipt`, `consultantProfile`, `orgBranding`, `paymentMethod`, `availabilitySchedule`, `availabilityRule`, `availabilityOverride`, `package`, `booking`, `bookingSlot`. Platform-level (NOT registered, accessed by super-admin/platform): `organization` (tenant root), `subscription`, `subscriptionPlan`, `feature_flags`, `billing_events`, `catalog_items`, `audit_logs`.

### `lib/` map (reuse these — don't re-implement)
- `db.ts` Prisma singleton · `tenant-db.ts` `tenantDb(orgId)` / `tenantTransaction()` · `oversight.ts` cross-tenant read · `rbac.ts` authz · `audit.ts` `writeAuditLog` · `otp.ts` · `slug.ts` (incl. `evaluateSlugInput` for the live field) · `money.ts` **pure** money (`computeEffectivePrice`/`formatMoney`/`parseFeatures` — client-safe) · `billing.ts` plan/subscription cores (re-exports money) · `checkout.ts` `totalSeatCount`/`buildCheckoutSummary` · `billing-engine.ts` webhooks/dunning · `gateway.ts`/`gateway-mock.ts` · `flags.ts` · `catalog.ts` · `consultants.ts` (org CRUD + `applyOrgStatus`) · `consultant-profile.ts` · `branding.ts` (contrast + branding cores) · `payments.ts` (UPI/gateway cores + `toSafeView` masking + `PaymentConnection`) · `crypto.ts` (envelope encrypt/decrypt) · `razorpay.ts` (test-connection) · `timezone.ts` (Intl tz conversions) · `availability.ts` (`computeFreeIntervals` + schedule cores) · `packages.ts` (package cores + `slugify`) · `scheduling.ts` (`getAvailableSlots` + atomic `reserveSlot`) · `admin-dashboard.ts` · `pdf.ts` (stub) · `email.ts`/`storage.ts`/`env.ts` (stubs/config).
