...
- **SP-2 COMPLETE.** SP-2 milestone met: consultant has branded identity + a working way to collect payment. **NEXT: SP-3 (scheduling engine)** — availability, packages, limits, booking questions, atomic slot reservation (GiST). Consultant nav items Packages/Bookings/Availability/Team/Seekers still "Soon" until SP-3+. (Deferred: OAuth "Connect" for gateways; real charge/order+webhook flow is SP-4.3; managed KMS later — master key is a Worker secret for now.)

### Tenant-scoped models registered (in `config/tenant-models.json`): `orgMember`, `receipt`, `consultantProfile`, `orgBranding`, `paymentMethod`. Platform-level (NOT registered, accessed by super-admin/platform): `organization` (tenant root), `subscription`, `subscriptionPlan`, `feature_flags`, `billing_events`, `catalog_items`, `audit_logs`.

### `lib/` map (reuse these — don't re-implement)
- `db.ts` Prisma singleton · `tenant-db.ts` `tenantDb(orgId)` / `tenantTransaction()` · `oversight.ts` cross-tenant read · `rbac.ts` authz · `audit.ts` `writeAuditLog` · `otp.ts` · `slug.ts` (incl. `evaluateSlugInput` for the live field) · `money.ts` **pure** money (`computeEffectivePrice`/`formatMoney`/`parseFeatures` — client-safe) · `billing.ts` plan/subscription cores (re-exports money) · `checkout.ts` `totalSeatCount`/`buildCheckoutSummary` · `billing-engine.ts` webhooks/dunning · `gateway.ts`/`gateway-mock.ts` · `flags.ts` · `catalog.ts` · `consultants.ts` (org CRUD + `applyOrgStatus`) · `consultant-profile.ts` · `branding.ts` (contrast + branding cores) · `payments.ts` (UPI/gateway cores + `toSafeView` masking + `PaymentConnection`) · `crypto.ts` (envelope encrypt/decrypt) · `razorpay.ts` (test-connection) · `admin-dashboard.ts` · `pdf.ts` (stub) · `email.ts`/`storage.ts`/`env.ts` (stubs/config).
